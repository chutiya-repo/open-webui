	...
	>
		{#if data.message.role === 'user'}
			<div class="flex w-full">
				<ProfileImage
					src={`${ANSWERAI_API_BASE_URL}/users/${data.user.id}/profile/image`}
		...
		{:else}
			<div class="flex w-full">
				<ProfileImage
					src={`${ANSWERAI_API_BASE_URL}/models/model/profile/image?id=${data.model?.id ?? data.message.model}&lang=${$i18n.language}`}
					className={'size-5 -translate-y-[1px]'}
				/>
