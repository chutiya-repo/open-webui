			...
			<div class="max-w-md">
				...
				>
					...
					{:else}
						...
						{$i18n.t('Contact Admin for ANSWERAI Access')}
					{/if}
				</div>
				...
				>
					...
					{:else}
						{$i18n.t('Your account status is currently pending activation.')}{'\n'}{$i18n.t(
							'To access the ANSWERAI, please reach out to the administrator. Admins can manage user statuses from the Admin Panel.'
						)}
					{/if}
				</div>
