...
from answerai.models.channels import (
    Channels,
    ChannelModel,
    ChannelForm,
    ChannelResponse,
...
from answerai.models.messages import (
    Messages,
    MessageModel,
    MessageResponse,
...
from answerai.utils.access_control import (
    has_access,
...
from answerai.utils.channels import extract_mentions, replace_mentions
...
async def send_notification(
    name, answerai_url, channel, message, active_user_ids, db=None
):
            ...
            if user.settings:
                ...
                if webhook_url:
                    await post_webhook(
                        name,
                        webhook_url,
                        f"#{channel.name} - {answerai_url}/channels/{channel.id}\n\n{message.content}",
                        {
                            "action": "channel",
                            "message": message.content,
                            "title": channel.name,
                            "url": f"{answerai_url}/channels/{channel.id}",
                        },
                    )
    ...
    try:
        ...
        async def background_handler():
            ...
            await send_notification(
                ...
                request.app.state.config.ANSWERAI_URL,
                channel,
                message,
                active_user_ids,
            )
