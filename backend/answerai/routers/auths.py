...
from answerai.env import (
    ...
    ANSWERAI_AUTH_TRUSTED_GROUPS_HEADER,
    ANSWERAI_AUTH_COOKIE_SAME_SITE,
...
from answerai.internal.db import get_session
...
from answerai.utils.groups import apply_default_group_assignment

from answerai.utils.redis import get_redis_client
from answerai.utils.rate_limit import RateLimiter
...
async def get_session_user(
    ...
    if data:
        ...
        response.set_cookie(
            ...
            samesite=ANSWERAI_AUTH_COOKIE_SAME_SITE,
            secure=ANSWERAI_AUTH_COOKIE_SECURE,
        )
...
async def update_password(
    ...
    if ANSWERAI_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(
    ...
    if ANSWERAI_AUTH_TRUSTED_EMAIL_HEADER:
        if ANSWERAI_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[ANSWERAI_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if ANSWERAI_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(ANSWERAI_AUTH_TRUSTED_NAME_HEADER, email)
            try:
        ...
        user = Auths.authenticate_user_by_email(email, db=db)
        if ANSWERAI_AUTH_TRUSTED_GROUPS_HEADER and user and user.role != "admin":
...
async def signout(
                            ...
                            if logout_url:
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f"&post_logout_redirect_uri={ANSWERAI_AUTH_SIGNOUT_REDIRECT_URL}"
                                            if ANSWERAI_AUTH_SIGNOUT_REDIRECT_URL
                                            else ""
                                        ),
                                    },
    ...
    if ANSWERAI_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                "status": True,
                "redirect_url": ANSWERAI_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def get_admin_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        "ADMIN_EMAIL": request.app.state.config.ADMIN_EMAIL,
        "ANSWERAI_URL": request.app.state.config.ANSWERAI_URL,
...
async def update_admin_config(
    ...
    request.app.state.config.ANSWERAI_URL = form_data.ANSWERAI_URL
