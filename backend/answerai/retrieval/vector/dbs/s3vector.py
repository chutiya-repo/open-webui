from answerai.retrieval.vector.utils import process_metadata
...
from answerai.config import S3_VECTOR_BUCKET_NAME, S3_VECTOR_REGION
...
class S3VectorClient(VectorDBBase):
    """
    AWS S3 Vector integration for ANSWERAI Knowledge.
    """
    ...
    def _filter_metadata(
        ...
        # Keep only the first 10 keys, prioritizing important ones based on actual ANSWERAI metadata
        important_keys = [
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            ...
            # The ANSWERAI GetResult expects lists of lists, so we wrap each list
            if all_ids:
                return GetResult(
