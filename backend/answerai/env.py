...
# answerai_DIR should be the directory where env.py resides (answerai/)
...
ANSWERAI_NAME = get_env("NAME", "ANSWERAI")
if ANSWERAI_NAME != "ANSWERAI":
    ANSWERAI_NAME += " (ANSWERAI)"

ANSWERAI_FAVICON_URL = "https://answerai.in/favicon.png"
...
ANSWERAI_BUILD_HASH = get_env("BUILD_HASH", "dev-build")
...
if os.path.exists(f"{DATA_DIR}/ollama.db"):
    # Rename the file
    os.rename(f"{DATA_DIR}/ollama.db", f"{DATA_DIR}/answerai.db")
    log.info("Database migrated from Ollama-ANSWERAI successfully.")
else:
    pass

DATABASE_URL = os.environ.get("DATABASE_URL", f"sqlite:///{DATA_DIR}/answerai.db")
...
if all(DB_VARS.values()):
    ...
    DATABASE_URL = f"sqlite+sqlcipher:///{DATA_DIR}/answerai.db"
...
####################################
# ANSWERAI_AUTH (Required for security)
####################################

ANSWERAI_AUTH = get_env("AUTH", "True").lower() == "true"
...
ANSWERAI_ADMIN_EMAIL = get_env("ADMIN_EMAIL", "")
ANSWERAI_ADMIN_PASSWORD = get_env("ADMIN_PASSWORD", "")
ANSWERAI_ADMIN_NAME = get_env("ADMIN_NAME", "Admin")

ANSWERAI_AUTH_TRUSTED_EMAIL_HEADER = get_env("AUTH_TRUSTED_EMAIL_HEADER", None)
ANSWERAI_AUTH_TRUSTED_NAME_HEADER = get_env("AUTH_TRUSTED_NAME_HEADER", None)
ANSWERAI_AUTH_TRUSTED_GROUPS_HEADER = get_env("AUTH_TRUSTED_GROUPS_HEADER", None)
...
ANSWERAI_AUTH_SIGNOUT_REDIRECT_URL = get_env("AUTH_SIGNOUT_REDIRECT_URL", None)

####################################
# ANSWERAI_SECRET_KEY
####################################

ANSWERAI_SECRET_KEY = get_env("SECRET_KEY", get_env("JWT_SECRET_KEY", "t0p-s3cr3t"))

ANSWERAI_SESSION_COOKIE_SAME_SITE = get_env("SESSION_COOKIE_SAME_SITE", "lax")
...
OAUTH_CLIENT_INFO_ENCRYPTION_KEY = os.environ.get(
    "OAUTH_CLIENT_INFO_ENCRYPTION_KEY", ANSWERAI_SECRET_KEY
)

OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = os.environ.get(
    "OAUTH_SESSION_TOKEN_ENCRYPTION_KEY", ANSWERAI_SECRET_KEY
)
